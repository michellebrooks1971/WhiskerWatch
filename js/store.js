...
(function () {
  ...
  function dueInfo(t) {
    ...
    if (days <= 7) return { state: 'soon', days: days, label: label };
  ...
  function recentLogs(daysBack) {
    const from = U.addDaysISO(U.todayISO(), -(daysBack || 14));
    return doc.logs
      .filter(function (l) { return l.date >= from; })
  ...
  function counts() {
    return {
      ...
      doses: doc.doses.length, treatments: doc.treatments.length, visits: doc.visits.length
    };
  }
  function isEmpty() {
    const c = counts();
  ...
  function addTreatment(data) {
    if (treatmentsForCat(data.catId).length >= MAX_TREATMENTS) {
      throw new Error('Each cat can have up to ' + MAX_TREATMENTS + ' treatments. Delete one first.');
    }
  ...
  function importJSON(text, importMode) {
    ...
    let added = 0;
    if (importMode === 'merge') {
      ['cats', 'logs', 'meds', 'doses', 'treatments', 'visits'].forEach(function (key) {
        const seen = {};
      ...
      added = incoming.cats.length + incoming.logs.length + incoming.meds.length +
              incoming.treatments.length + incoming.visits.length;
    }
    save('import');
    return added;
  }
  ...
  function loadSample() {
    ...
    const cats = [
      ...
      normCat({ id: 'sample-c2', name: 'Biscuit', dob: '2019-07-02', sex: 'male', breed: 'Ginger tabby', colour: CAT_COLOURS[4], notes: 'Food-obsessed — watch his weight.' }),
  ...
  // ---------- expose ----------
  window.Store = {
    KINDS: KINDS, UNITS: UNITS, APPETITES: APPETITES,
    MAX_TREATMENTS: MAX_TREATMENTS, CAT_COLOURS: CAT_COLOURS,
    get doc() { return doc; },
    ...
    logFor: logFor, getLog: getLog, recentLogs: recentLogs,
